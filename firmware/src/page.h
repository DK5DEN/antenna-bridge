...
<div id="ants"></div>
<div class="row"><button class="acc" onclick="$('antdlg').showModal();$('dn').focus()">Add antenna</button><button onclick="cmd('ant select -')">None active</button></div>
<dialog id="antdlg"><h2>New antenna</h2>
<div class="grid2"><div><label>Name</label><input id="dn" placeholder="efhw" maxlength="15" style="width:100%"></div><div><label>Type</label><select id="dt" style="width:100%"><option>efhw</option><option>dipole</option><option>vertical</option><option>loop</option><option>beam</option><option>wire</option><option>other</option></select></div></div>
<label>Direct ranges, no tuner (optional)</label><div class="row" id="dbands"></div>
<div class="row"><span id="dbandsel"></span><input id="dmin" type="number" step="0.1" placeholder="from kHz" style="width:110px"><input id="dmax" type="number" step="0.1" placeholder="to kHz" style="width:110px"><button onclick="dlgBandAdd()">Add range</button></div>
<div class="row" style="justify-content:flex-end"><button onclick="$('antdlg').close()">Cancel</button><button class="acc" onclick="dlgCreate()">Create</button></div>
</dialog>
</section>
...
let dlgBands=[];
function dlgBandAdd(){const a=Math.round(v('dmin')*1000),b=Math.round(v('dmax')*1000);if(!a||!b||a>b)return;dlgBands.push([a,b]);$('dmin').value='';$('dmax').value='';dlgRender();}
function dlgRender(){$('dbands').innerHTML=dlgBands.map((b,i)=>'<span class="chip on" onclick="dlgBands.splice('+i+',1);dlgRender()" title="remove">'+bandName(b[0],b[1])+' ✕</span>').join('')||'<span class="tag">none</span>';if(!$('dbandsel').innerHTML)$('dbandsel').innerHTML=bandSel('dmin','dmax');}
async function dlgCreate(){const n=v('dn').trim();if(!n||/\s/.test(n)){alert('Name: 1-15 characters, no spaces');return;}await cmd('ant add '+n+' '+v('dt'));for(const b of dlgBands)await cmd('ant band '+n+' add '+b[0]+' '+b[1]);dlgBands=[];$('dn').value='';dlgRender();$('antdlg').close();poll();}
let editAnt=null;
...
g+=fo(rigX+10,midY-16,rigW-20,36,'<select style="width:100%;height:28px;font-size:12px;padding:0 .4rem" onchange="cmd(\'rig set \'+this.value)">'+s.rigs.map(r=>'<option value="'+r.id+'"'+(r.id===s.settings.rig?' selected':'')+'>'+esc(r.name)+(r.stored?' (stored)':'')+'</option>').join('')+'</select>');
...
g+=fo(antX+10,midY-16,antW-20,36,'<select style="width:100%;height:28px;font-size:12px;padding:0 .4rem" onchange="cmd(\'ant select \'+this.value)"><option value="-"'+(ant?'':' selected')+'>no antenna</option>'+s.ants.map(a=>'<option value="'+esc(a.name)+'"'+(a.name===ant?' selected':'')+'>'+esc(a.name)+(a.type?' ('+esc(a.type)+')':'')+'</option>').join('')+'</select>');
