...
<div class="row"><label style="margin:0">Rig</label><select id="rigsel2" style="width:260px" onchange="cmd('rig set '+this.value)"></select><label style="margin:0 0 0 12px">Antenna</label><select id="antsel" style="width:200px" onchange="cmd('ant select '+this.value)"><option value="-">none</option></select></div>
<p class="tag">Live picture: the rig reports the frequency, the active antenna's rules (and the global ones, dashed) decide the outputs. Green = on, red = forced off by an OFF rule, grey = no rule for this frequency. Rig and antenna are chosen below (or click an antenna). Edit the rules on the Map, Matrix or Rules tab.</p>
</section>
...
g+='<g><rect class="node" x="10" y="'+rigY+'" width="190" height="64" rx="8"/><text x="20" y="'+(rigY+20)+'">'+esc(s.cat.rigname||s.settings.rig)+'</text><text class="sub" x="20" y="'+(rigY+36)+'">'+(s.cat.ok?'<tspan fill="#10b981">CAT linked</tspan>':(s.cat.rigname==='No CAT, frequency from the network'?'network':'<tspan fill="#ef4444">no CAT link</tspan>'))+' · '+esc(s.src)+'</text><text x="20" y="'+(rigY+56)+'" style="font-size:16px;font-weight:600">'+(s.freq?khz(s.freq)+' kHz':'–')+'</text></g>';
const r2=$('rigsel2');const rk2=s.rigs.map(r=>r.id).join(',');if(r2.dataset.k!==rk2){r2.dataset.k=rk2;r2.innerHTML=s.rigs.map(r=>'<option value="'+r.id+'">'+esc(r.name)+(r.stored?' (stored)':'')+'</option>').join('');}if(document.activeElement!==r2)r2.value=s.settings.rig;
const as=$('antsel');const ak=s.ants.map(a=>a.name).join(',');if(as.dataset.k!==ak){as.dataset.k=ak;as.innerHTML='<option value="-">none</option>'+s.ants.map(a=>'<option value="'+esc(a.name)+'">'+esc(a.name)+(a.type?' ('+esc(a.type)+')':'')+'</option>').join('');}if(document.activeElement!==as)as.value=s.active||'-';
