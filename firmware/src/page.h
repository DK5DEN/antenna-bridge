...
<div class="row" id="rigpick" hidden><span class="tag">Rig:</span><span id="rigchips"></span></div>
...
$('ants').innerHTML=s.ants.map(a=>'<div class="out"><div><div class="n">'+esc(a.name)+(a.name===s.active?' <span class="ok">● active</span>':'')+'</div><div class="t">'+esc(a.type||'')+'</div></div><div class="tag" style="flex:1">'+(s.outs.filter(o=>o.used.includes(a.name)).map(o=>esc(o.name)).join(', ')||'no rules yet')+'<br>direct: '+(a.bands.length?a.bands.map(b=>bandName(b[0],b[1])).join(', '):'<span class="tag">not set</span>')+'</div><div>'+(a.name===s.active?'':'<button class="sm acc" onclick="cmd(\'ant select '+a.name+'\')">activate</button>')+'<button class="sm" onclick="antEdit(\''+a.name+'\')">edit</button><button class="sm err" onclick="if(confirm(\'Remove antenna '+a.name+' and its rules?\'))cmd(\'ant del '+a.name+'\')">✕</button></div></div>'+(editAnt===a.name?antEditor(a):'')).join('')||'<span class="tag">no antennas yet</span>';}
